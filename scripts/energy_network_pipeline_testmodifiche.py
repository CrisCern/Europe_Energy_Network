...
class NodeRemovalImpactAnalyzer:
    ...
    def _get_critical_node(self):
        betw = pd.read_csv(self.betweenness_path)
    ...
    def analyze_removal_impact(self, num_random_nodes=10):
        random.seed(self.seed)
        critical_node = self._get_critical_node()
        original_length = self._compute_average_shortest_path(self.G)

        # Rimuovi nodo critico
        G_critical = self.G.copy()
        G_critical.remove_node(critical_node)
        critical_length = self._compute_average_shortest_path(G_critical)

        # Rimuovi nodi casuali
        random_lengths = []
        random_nodes = random.sample([n for n in self.G.nodes if n != critical_node], num_random_nodes)
        for node in random_nodes:
            G_temp = self.G.copy()
            G_temp.remove_node(node)
            l = self._compute_average_shortest_path(G_temp)
            random_lengths.append((node, l))

        # Salvataggio risultati
        df = pd.DataFrame({
            "type": ["original", "critical"] + ["random"] * num_random_nodes,
            "node": [""] + [critical_node] + [n for n, _ in random_lengths],
            "avg_shortest_path_length": [original_length, critical_length] + [l for _, l in random_lengths]
        })
        ...
        plt.bar(df["type"], df["avg_shortest_path_length"], color=["gray", "red"] + ["lightblue"] * num_random_nodes)
        plt.title("Confronto dell'impatto della rimozione dei nodi")
...
if __name__ == "__main__":
    ...
    impact_analyzer = NodeRemovalImpactAnalyzer(
        graph=G_2024,
        betweenness_path="../metrics_2024/betweenness_2024.csv",
        output_csv="../metrics/removal_impact_2024.csv",
        output_png="../figures/removal_impact_2024.png",
        seed=42
    )
    impact_analyzer.analyze_removal_impact(num_random_nodes=10)
